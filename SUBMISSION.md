...
This is a local foundation, not a live trading product. No mainnet execution and no funded live wallets. PreStocks and Tessera appear as labeled recipe previews from their documented catalogs; those mints are not created on localnet and previews are not fills.
...
| PreStocks bounty | Recipe-preview rail from `GET https://prestocks.com/api/prestocks` (or labeled recorded catalog). Not a live basket, not mixed with Tessera. |
| Tessera bounty | Recipe-preview rail from `GET https://rest-api.tessera.pe/v1/public/token-details`, including T-OpenAI and T-Kalshi. Not a live basket. Tessera Token-2022 extensions still fail `create_basket`. |
...
- A PreStocks/Tessera create, swap, or fill on localnet
