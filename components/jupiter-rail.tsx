    ...
    <section className="mt-8 rounded-[6px] border border-black/10 bg-white">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-black/10 px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          <span className="border border-black/15 px-2 py-1 text-[11px] font-medium text-[var(--muted)]">
            {sourceBadge}
          </span>
          <button
      ...
      <div className="space-y-3 px-4 py-4 text-sm">
        ...
        <div className="grid gap-3 border-t border-black/10 pt-4 md:grid-cols-[1fr_auto] md:items-end">
          <label className="block text-xs font-semibold uppercase text-[var(--muted)]">
            Quote amount
            <input
              className="mt-2 h-11 w-full rounded-[6px] border border-black/15 px-3 font-mono text-sm outline-none focus:border-[var(--blue)]"
              inputMode="decimal"
        ...
        <p className={`text-xs ${amountError ? "text-[var(--red)]" : "text-[var(--muted)]"}`}>
          {amountError
            || (funding
        ...
        {quote ? (
          <div className="rounded-[6px] border border-black/10 bg-[#f7f8f5] p-3">
            <div className="flex items-start gap-2">
              <ShieldCheck className="mt-0.5 text-[var(--green)]" size={16} />
              <div className="min-w-0 text-xs leading-5">
                <p className="font-semibold">
                  {quote.error ? "Jupiter quote unavailable" : "Jupiter quote (not a fill)"}
                </p>
                {quote.error ? (
                  <p className="mt-1 text-[var(--muted)]">{quote.error}</p>
                ) : (
                  <ul className="mt-1 space-y-1 font-mono text-[11px] text-[var(--muted)]">
  ...
  return (
    <div>
      ...
      <div className="max-h-56 overflow-auto rounded-[6px] border border-black/10">
        {assets.length === 0 ? (
          <p className="px-3 py-4 text-xs text-[var(--muted)]">None from Jupiter.</p>
        ) : assets.map((asset) => {
