...
- **PreStocks** as a PreStocks-only recipe-preview rail from the documented PreStocks catalog. Previews are not creates, not fills, and not Pyth marks. Non-PreStocks pre-IPO tokens (including Tessera) are omitted from that path; Tessera is a related link-out only. Localnet create stays on issued test mints.
...
| PreStocks recipes | PreStocks-only preview from issuer catalog or labeled **FIXTURE**; empty if **UNAVAILABLE** | A localnet create, a fill, a Pyth mark, or a Tessera/other pre-IPO mix |
...
7. **Recipe rail** — scroll to **PreStocks recipes**. Badge **PRESTOCKS LIVE** when `GET /api/prestocks` responds, or **FIXTURE** / **UNAVAILABLE**. Select a PreStocks recipe (for example OpenAI + Anthropic). Success is labeled **PreStocks recipe preview (not a create, not a fill)**. Localnet create stays **N/A**. Tessera is a related link-out only — it is not a recipe on this path.
...
- Creating a PreStocks basket on this validator
- Mixing Tessera or other non-PreStocks pre-IPO tokens into the PreStocks recipe path
